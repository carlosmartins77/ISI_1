...
public class DB : IDBSoap, IDBRest
{
    ...
    public bool AddProductToOrder(Encomenda e)
    {
        ...
        // "INSERT INTO EncomendaProduto(ID_ProdutoFK, ID_EncomendaFK, Quantidade, Total) VALUES(@ID_Produto, (SELECT TOP 1 ID_Encomenda FROM Encomenda ORDER BY ID_Encomenda DESC), @Quantidade, Quantidade*Preco)";
        // Outra query para UPDATE EncomendaProduto SET Total = Total1 + Total (
        // Executar
    ...
    public bool AddProductToOrder2(Encomenda e)
    {
        DataSet ds = new DataSet();
        ...
        // Query para adicionar o produto à ultima instancia de encomenda criada.
        string queryAdicionarProduto = "INSERT INTO EncomendaProduto(ID_ProdutoFK, ID_EncomendaFK, Quantidade, Total) VALUES(@ID_Produto, (SELECT TOP 1 ID_Encomenda FROM Encomenda ORDER BY ID_Encomenda DESC), @Quantidade)";

        string updatepreco = "UPDATE EncomendaProduto SET Total = Total1 + Total";
        // Executar
        SqlDataAdapter da = new SqlDataAdapter(queryAdicionarProduto, con);
    ...
    public bool UpdatePrice(double preco, string sku)
    {
        ...
        //// Query para encontrar o produto
        //SqlDataAdapter daFind = new SqlDataAdapter
        //{
        //    SelectCommand = new SqlCommand("SELECT Preco FROM PRODUTO WHERE SKU = @SKU", con)
        //};

        //daFind.SelectCommand.Parameters.Add("@SKU", SqlDbType.NVarChar).Value = sku;
        ...
        daUpdate.SelectCommand.Parameters.Add("@SKU", SqlDbType.NVarChar).Value = sku;
        daUpdate.SelectCommand.Parameters.Add("@SKU", SqlDbType.Float).Value = preco;

        try
        {
            daUpdate.Fill(ds, "Produto"); //Adicionar à tabela
            return true;
        }
        catch (Exception)
        {
    ...
    public void RegistIsolated(string nif)
    {
        ...
        using (SqlConnection con = new SqlConnection(ConfigurationManager.ConnectionStrings["tpISIConnectionString"].ConnectionString))
        {
            ...
            SqlDataAdapter da = new SqlDataAdapter
            {
                UpdateCommand = new SqlCommand("UPDATE Pessoa SET Testado = 0, Infetado = 0, Isolamento = 1 WHERE Nif = @nif", con)
            };
    ...
    /// </summary
    /// <returns>Um dataset com a lista de produtos mais requisitados</returns>
    ...
    /// <summary>
    /// Função responsavel contar o número de testados
    /// </summary
    /// <returns>Um dataset com o número de testados</returns>
    public DataSet GetTestados()
    {
        DataSet ds = new DataSet();
        ...
        // Contar o numero de pessoas testadas
        string query = @"SELECT COUNT(Testado) as NumeroDeTestados
                        FROM Pessoa
                        WHERE Testado = 1";
    ...
    /// <summary>
    /// Função responsavel contar o número de infetados
    /// </summary
    /// <returns>Um dataset com o número de infetados</returns>
    public DataSet GetInfetados()
    {
        DataSet ds = new DataSet();
        ...
        // Contar o numero de pessoas testadas
        string query = @"SELECT COUNT(Infetado) as NumeroDeInfetados
                        FROM Pessoa
                        WHERE Infetado = 1";
